Shader "Custom/Polka-Dots"
{
    Properties
    {
        _MainTex("Color LUT", 2D) = "white" {}
        _WorleyScale("Worley Scale", Float) = 10.0
        _DotSize("Polka Dot Size", Range(0.0, 1.0)) = 0.5
    }
    SubShader
    {
        ...
        void surf(Input IN, inout SurfaceOutputStandard o)
        {
            ...
            //apply step to distance combination to create dots with distance from feature point
            //equal to _DotSize
            float2 uv = float2(step(1-distanceComb, _DotSize), 0.5);

            //for color variation of dots
            if (uv.x == 0.0)
                uv.x = noiseData.f1_id;

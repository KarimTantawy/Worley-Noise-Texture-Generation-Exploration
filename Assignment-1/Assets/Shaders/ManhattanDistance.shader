Shader "Custom/ManhattanDistance"
{
    Properties
    {
        _MainTex ("Color LUT", 2D) = "white" {}
        _WorleyScale("Worley Scale", Float) = 10.0
        _StepThreshold("Step Threshold", Range(0.0, 1.0)) = 1.0
    }
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            float distanceComb = noiseData.f4;

            //normalizing distance to be in range of [0.0, 1.0]
            distanceComb = smoothstep(0.0, 3.0, distanceComb);

            //use Color LUT to determine color of pixel
            float2 uv = float2(noiseData.f1_id, 0.5);
           
            uv.x *= step(distanceComb, _StepThreshold);

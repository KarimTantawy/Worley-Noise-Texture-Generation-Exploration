Shader "Custom/SitePlacementExample"
{
    Properties
    {
        _MainTex("Color LUT", 2D) = "white" {}
        _WorleyScale("Worley Scale", Float) = 10.0
        _DotSize("Site Size", Range(0.0, 1.0)) = 0.5
    }
    SubShader
    {
        ...
        float2 randomNumGenerator(float2 gridPosition)
        {
            ...
            void surf(Input IN, inout SurfaceOutputStandard o)
            {
                ...
                //display sites
                float color = 1 - step(distanceComb, _DotSize);

                //get the position of the pixel within the grid
                float2 posInGrid = frac(scaledUV);

                //step with lineWidth to produce lines
                float mValX = step(lineWidth, posInGrid.x);
                float mValY = step(lineWidth, posInGrid.y);

                //determine the final color by checking if pixel
                //is black from grid in X axis or grid in Y axis
                color = min(color, mValX);
                color = min(color, mValY);

                fixed4 c = fixed4(color, color, color, 1.0);

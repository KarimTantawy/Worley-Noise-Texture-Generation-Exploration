        ...
        SubShader
        {
            ...
            float2 randomNumGenerator(float2 gridPosition)
            {
             ...
             void surf(Input IN, inout SurfaceOutputStandard o)
             {
                 ...
                 float distanceComb1 = noiseData.f2 - noiseData.f1;
                 float distanceComb2 = noiseData.f1;

                 float color1 = 1.0;
                 float color2 = 0.0;

                 float color = lerp(color1, color2, distanceComb1);
                 float colorb = lerp(color1, color2, distanceComb2);
                 float finalColor = step(0.8, color) * color;

                 if(step(0.9, colorb) * colorb != 0)
                     finalColor = step(0.9, colorb) * colorb;

                 //use Color LUT to determine color of pixel
                 float2 uv = float2(finalColor, 0.5);

                 //clamping values because edges of LUT texture are gray and affect the color gradien
                 uv.x = clamp(uv.x, 0.01, 0.99);

                 fixed4 c = tex2D(_MainTex, uv);

                 //fixed4 c = fixed4(distanceComb, distanceComb, distanceComb, 1.0);
